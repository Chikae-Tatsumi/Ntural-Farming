...
# Main + <1% abund 
majors <- agrregated[agrregated[,"rowMeans"] > 0.5,] 
majors<-majors[!(rownames(majors)=="Unknown_Family"),]
majors <- majors[order(majors$rowMeans,decreasing = T),]
minors <- agrregated[agrregated[,"rowMeans"] < 0.5,] 
...
table.d <- data.frame(table)
colnames(table.d)<-colnames(table)
data <- data.frame()
for (i in 3:(ncol(table.d))){
Abundance <-table.d[,i]
Example <- colnames (table.d)[i]
...
# Save
ggsave(file = "Rel.Abund.Family.png", width=8.2, height =5)
