...
se <- spiec.easi(subset.t.filter, method='glasso', lambda.min.ratio=1e-2,
                          nlambda=50) 
saveRDS(se, paste("Network/SpiecEasi/0.02/bacteria/",NAME[i],".se.rds", sep=""))
...
write.csv(data, paste("Network/SpiecEasi/0.02_inAll/bacteria/",NAME[i],".metrics.csv", sep=""))
Results <- rbind (Results,data)
}
write.csv(Results, "Network/SpiecEasi/0.02_inAll/bacteria/all.metrics.csv")
