...
subset.t.filter <- subset[ ,colMeans(percent.t) >= 0.02] 
...
saveRDS(tax.info, paste("Network/SpiecEasi/0.02_inAll/fungi_bacteria/",NAME[i],".tax.info.rds", sep=""))
print(c(ncol(subset),"versus",ncol(subset.t.filter)))

# Calculate network (spiec.easi)
se <- spiec.easi(subset.t.filter, method='glasso', lambda.min.ratio=1e-2, nlambda=50) 
saveRDS(se, paste("Network/SpiecEasi/0.02/fungi_bacteria/",NAME[i],".se.rds", sep=""))
...
write.csv(data, paste("Network/SpiecEasi/0.02/fungi_bacteria/",NAME[i],".metrics.csv", sep=""))
Results <- rbind (Results,data)
}
write.csv(Results, "Network/SpiecEasi/0.02/fungi_bacteria/all.metrics.csv")
