...
minor.phylum <- phylum[phylum[,"rowMeans"] < 2,] # Change
...
legend(x = -2, y = 1.5, legend = levels(sel.tax$Phylum), cex=2, pch = 19, col = col, bty = "n",  y.intersp = 0.05, x.intersp = 0.05)
text(x=0,y=-0.8,paste("Edge: ", format(gsize,digit=3)),cex=3)
text(x=0,y=-0.9,paste("Density: ", format(edge_density,digit=3)),cex=3)
title(NAME[i],cex=2) #Change

# Save 
dev.copy(pdf, file=paste("Network/Network.HNPF.16S.",NAME[i],".pdf",sep=""), height=5, width=8)
dev.off()
dev.copy(png, file=paste("Network/Network.HNPF.16S.",NAME[i],".png",sep=""), height=800, width=800)
dev.off()}
...
write.csv(Results, "Network/network.indicates.season.composited.csv")
